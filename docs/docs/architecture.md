...
- `ipc/app.ts`(v0.2.2): `requestAppActivation()`(`activate_app` を即時+`ACTIVATION_RETRY_DELAY_MS`後に1回呼ぶ。失敗は`console.warn`のみ)。テキスト入力欄のフォーカス時に`main.ts`が`bindTextTool(canvas, { onEditorFocus })`経由で呼ぶ(canvas層からIPC層へ直接依存しない)。Dock非表示のアプリが非アクティブのままだと日本語IMEが効かないための対策
