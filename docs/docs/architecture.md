...
│   ├── shapeEdit.ts          # 編集中の図形の当たり判定・リサイズ・移動等の純粋関数(T31)
│   ├── pendingShape.ts       # 編集中の図形(直前の1つ)の保持・確定・破棄(T31)
│   └── tools/
...
│   ├── pendingShapeKeys.ts   # 編集中の図形のEnter確定/Esc破棄(T31)
...
| `src/canvas/undoStack.test.ts` | `createUndoStackState()`/`withPushedUndoStep()`(push・Redoクリア・イミュータブル・上限30件超過時の破棄)/`withPoppedUndo()`/`withPoppedRedo()`(LIFO取り出し・対になるスタックへ`currentImage`を積む・空スタックは`null`・Redo側の上限)/`canUndoState()`/`canRedoState()`(純粋関数)、`getUndoStackState()`/`pushUndoStep()`/`popUndo()`/`popRedo()`/`canUndo()`/`canRedo()`/`clearUndoStack()`/`subscribeUndoStack()`(シングルトンストア、push/popの対応関係・popUndo後のpushUndoStepでRedoクリア・popRedoの対称動作・clearUndoStackでの両スタック初期化・通知/購読解除を含む)(T23、FR-014) |
...
| `shapeEdit.test.ts` | `src/canvas/` | 編集中の図形(T31): `createShapeFromDrag()`・`getShapeHandles()`・`hitTestShape()`(ハンドル優先・矩形/楕円の内側・矢印の胴体)・`resizeShape()`(対角固定・反転時の正規化・Shift正方形・最小サイズ未満は据え置き・Canvasクランプ)・`moveShape()`(はみ出さないよう移動量をクランプ)・`applyEditDrag()`・`decidePointerDown()`(編集/確定して作成/確定のみ/無視)・`shapeUndoRect()`・`cursorForHit()` |
| `pendingShape.test.ts` | `src/canvas/` | 編集中の図形ストア(T31): 作成→リサイズ→移動→確定で確定時に1回だけUndoへ積む、破棄はUndoへ積まずbaseを復元、確定は復元しない、画像差し替え時は書き戻さず捨てる、新しい図形の開始時に既存を確定、購読通知 |
...
| `pendingShapeKeys.test.ts` | `src/ui/` | `pendingShapeKeyAction()`(編集中のみEnter=確定・Esc=破棄、修飾キー・IME変換中・入力欄では奪わない)(T31) |
...
| `e2e/undo-redo.spec.ts` | T29: 矢印→矩形→モザイクをCmd+Z×3で元画像と完全一致・Cmd+Shift+Z×3で描いた後と一致・ボタンでも1操作ずつ/編集中の図形はCmd+Z・ボタンで破棄されやり直し対象にならない/編集中はやり直し無効/テキスト入力中のCmd+ZはCanvasに作用しない/新規キャプチャでスタックが空になる |
...
| `e2e/shape-edit.spec.ts` | T31: 編集中の図形はコピー時に確定されて写りハンドル(白)は写らない(コピーRGBA=Canvas、近白画素0)/矩形の右下ハンドルでリサイズ/矢印の胴体ドラッグで移動+Enter確定/Escで破棄/次の図形の描き始めで直前の図形が確定 |
