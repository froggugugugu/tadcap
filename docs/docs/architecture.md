...
| `src/history/documentArchive.test.ts` | T34: `commandPixelBytes`(pixels・flatten・groupの中)、`trimUndoToBudget`(上限以内はそのまま/古い取り消し→遠いやり直しの順に連続して捨てる)、8MB上限、履歴idごとの保存・取得・削除と保存時の上限適用 |
