...
- **描いた後も調整できる**: 矢印・矩形・円・テキストはクリックで選び直して移動、ハンドルでリサイズ(テキストはダブルクリックで打ち直し)。選択中は色・文字サイズの変更、`Delete` で削除、`⌘⇧F` / `⌘⇧B` で最前面・最背面へ。`Shift` で正方形・正円。1 枚の画像に 50 個まで(超えると古いものから画像に固定)。履歴を切り替えて戻っても続きから調整できる
...
3. 画像の上をドラッグして描く。テキストはクリックした位置に入力して `Enter`。描いたものはクリックで選び直して調整できる
4. `⌘C` でクリップボードにコピー
5. 貼り付けたい場所で `⌘V`
...
| `Enter` / `Esc` | 選択を外す(テキスト入力中は `Enter` で確定、`Esc` で取り消し) | エディタ |
| `Delete` / `⌫` | 選択中のオブジェクトを削除 | オブジェクト選択中 |
| `⌘⇧F` / `⌘⇧B` | 選択中のオブジェクトを最前面 / 最背面へ | オブジェクト選択中 |
| ダブルクリック | テキストを打ち直す | テキストの上 |
