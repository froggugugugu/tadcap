...
| 項目 | 内容 |
| ---- | ---- |
| 最終更新 | 2026-09-26(v0.2.2 公開済み。日本語入力は人間の実機で解消を確認) |
...
### 2026-09-26 — v0.2.2 リリース・日本語入力解消

- **v0.2.2**: ウィンドウのフォーカス時とテキスト入力欄のフォーカス時に NSApplication::activate(`ensure_app_active`・`activate_app` コマンド、入力欄は 150ms 後に 1 回再試行)。vitest 396 / cargo 113 / e2e 49。Releases に DMG/zip、インストーラーで 0.2.2 導入・codesign OK
- **人間の確認**: 一度「何も入らない(Google 日本語入力)」と報告後、「治ってるっぽい」→ 解消と判断(原因=非アクティブの Accessory アプリに IME が効かない)
- **中止**: 診断ログ(`TADCAP_DEBUG_INPUT`)の作成は不要になったため停止し、途中の変更を破棄(HEAD と一致を確認)

### 2026-09-26 — 日本語入力できない不具合

- **人間の報告(v0.2.1)**: テキストに全角文字を入力できない
- **調査**: Web 層(keydown ハンドラ・入力欄・CSS)は原因でないことを実 WebKit ハーネスで確認。非アクティブな Accessory アプリに keyDown を送ると IME が効かず半角が入る → 推定原因=撮影後にアプリが非アクティブのまま前面化されている
- **対応**: ウィンドウのフォーカス時・テキスト入力欄のフォーカス時に NSApplication::activate する修正を v0.2.2 として配布し、人間が配布版で確認(切り分け A・B)

### 2026-09-26 — v0.2.0 / v0.2.1 リリース

- **v0.2.0**: 確定後の再調整(T32〜T34)。Releases に DMG/zip、インストーラーで導入・codesign OK・版 0.2.0
- **v0.2.1**(人間の要望): トースト自動消去(成功 2.5 秒・エラー 5 秒)、矩形の角丸(半径 = min(線幅×2.5, 短辺×0.25))、履歴上限 20 件+合計約 300MB(実測バイト)。vitest 392 / e2e 46。Releases に DMG/zip、インストーラーで導入・codesign OK・版 0.2.1

### 2026-09-25 — T33・T34 完了、v0.2.0

- **T33**: テキストもオブジェクト化(選択・移動・ダブルクリックで再編集、空で確定=削除)
- **T34**: Delete 削除・選択中の色/文字サイズ変更・最前面/最背面(⌘⇧F / ⌘⇧B)・履歴ごとにベース PNG+オブジェクト+取り消しスタックを退避/復元
- **検証(PJM 再実行)**: build 0 / vitest 0(372) / cargo test 0(109) / clippy 0 / e2e 0(41) / dist:mac 0・codesign OK・版 0.2.0
- **README・紹介ページ**: 新しい操作とショートカット表を更新
- **実機未確認(開発者申告)**: WKWebView での createImageBitmap(Blob)、macOS カラーパネルの change 発火

### 2026-09-25 — T32 完了

- **やったこと**: `objectModel`/`commands`/`documentState`/`documentSurface`(ベース canvas + オブジェクト配列、毎回 ベース drawImage + オブジェクト描画)、クリックで選択→移動/リサイズ、上限 50(51 個目で最古をベースへ焼き込み、追加と同じ group で 1 回の取り消し)、コマンド add/update/remove/pixels/flatten/group。モザイク・テキストはベース加工(オブジェクトより下)。pendingShape 系を廃止
- **検証(PJM 再実行)**: build 0 / vitest 0(335) / cargo test 0 / clippy 0 / e2e 0(32)。5K×50 個で約 60fps
- **次**: T33(テキストのオブジェクト化)→ T34 → v0.2.0

### 2026-09-24 — 確定後の再調整(オブジェクト化)の要望

- **人間(v0.1.2 実機確認)**: 「いい具合」。確定後も再調整したい、上限を設けてよい
- **決定(AskUserQuestion)**: 対象=矢印・矩形・円・テキスト(モザイクは画像加工のまま、取り消しで戻す)/ 上限 1 画像 50 個、超えたら古い順に焼き込んで固定 / 履歴を切り替えて戻っても再調整可(履歴ごとに元画像+オブジェクト一覧をメモリ保持)/ 選択中の操作: Delete 削除・色変更・文字サイズ変更・前面/背面
- **計画**: T32(オブジェクト層の基盤: モデル・描画・選択・移動/リサイズ・上限・取り消し/やり直しのコマンド化・PRD/ARCH 改訂)→ T33(テキストのオブジェクト化: 移動・ダブルクリック再編集)→ T34(選択中の操作: 削除・色・文字サイズ・前面/背面、履歴ごとの保持、E2E)→ v0.2.0
